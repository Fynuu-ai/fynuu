...
const InteractiveFynuuDemo = () => {
    ...
    switch (currentStep) {
      ...
      case 3:
        ...
        return <div className="h-full flex flex-col bg-gradient-to-b from-primary/5 to-black relative overflow-hidden">
            ...
            {/* App Icons Grid */}
            <div className="absolute top-20 left-0 right-0 grid grid-cols-4 gap-6 px-8">
              {['📷', '📞', '✉️', '🎵', '📱', '🌐', '⚙️', '🎮'].map((emoji, i) => <div key={i} className="w-14 h-14 bg-white/5 backdrop-blur-sm rounded-2xl flex items-center justify-center text-2xl">
                  {emoji}
                </div>)}
            </div>
  ...
  return <div className="relative flex flex-col items-center">
      {/* Phone Mockup */}
      <div className="phone-mockup w-80 h-[640px] relative overflow-hidden bg-black">
